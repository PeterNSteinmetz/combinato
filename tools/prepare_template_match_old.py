...
def main(preIctal_session, total_session, preIctal_label, joblist):#, channels):
    ...
    channel_old=[]
    for job in joblist:
        if channel_old==job[0][:job[0].rfind('/')]:
            artifact_helper+=1
        else:
            artifact_helper=0
    ...
    #for channel in channels:
        ...
        transform_sorting(sorting_PreIctal_files, sorting_TemplateTimes_file[0], preIctal_file, totalSession_file, artifact_helper)
...
def transform_sorting(sorting_PreIctal_files, sorting_TemplateTimes_file, preIctal_file, totalSession_file, artifact_helper): # input is clustered sorting.h5 of PreIctal and unclustered sorting.h5 of new Data
...
# convert the index, class and match from the preIctal data (ranging usually from 0 to ...) to the total data (usually no starting at 0 but somewhere between 1000 and 3000)
        ...
        if help==artifact_helper:
            artifacts_total=np.zeros((len(sorting_PreIctal.root.artifact_scores[:]),2),np.uint8)
            artifacts_total=sorting_PreIctal.root.artifact_scores[:]
        #artifacts_total=np.append(artifacts_total[:],sorting_PreIctal.root.artifact_scores[:],axis=0)
        sorting_PreIctal.close()
    #artifacts_total=unique_rows(artifacts_total)
